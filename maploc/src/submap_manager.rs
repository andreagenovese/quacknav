...
pub struct SubmapManager {
    ...
    /// Set by [`SubmapManager::request_switch`]; cleared by the tick that
    /// acts on it.
...
impl SubmapManager {
    ...
    /// Freeze the current submap at the next tick, whatever its age or
    /// travel. Called when the tracked pose has just been corrected by
    /// more than the ink can absorb: what is already drawn in this submap
    /// was drawn at the old pose, and a submap is rigid, so no later
    /// optimisation can ever separate the two. Cutting here bounds the
    /// damage to what is already written.
