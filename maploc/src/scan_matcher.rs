...
pub struct ScanMatchResult {
    ...
    /// Gauss-Newton normal matrix J^T J at the final pose over the beams
    /// that contributed. Its translational 2x2 block tells a caller how well
    /// the scene constrains the pose: a single straight wall leaves one
    /// eigenvalue near zero, and a correction along that eigenvector is
    /// sliding, not localization.
    pub hessian: [[f32; 3]; 3],
...
pub fn match_scan(
    ...
    for &(bx, by) in &endpoints {
        ...
        let js = [
            dd_dx,
            dd_dy,
            dd_dx * (-sin_t * bx - cos_t * by) + dd_dy * (cos_t * bx - sin_t * by),
        ];
        for a in 0..3 {
            for b in 0..3 {
                hessian[a][b] += js[a] * js[b];
            }
        }
    }
