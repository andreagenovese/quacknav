...
pub struct AccumulatorConfig {
    ...
    pub min_frames: u32,
    /// Beams longer than this are dropped entirely — past some distance
    /// the sensor's noise costs more than the coverage buys.
    ///
    /// Where that distance is, measured on the twin (2026-09-10): three
    /// metres, not two. Replaying eight recorded sessions across three
    /// simulated flats and scoring every resulting map against the house's
    /// own walls, the median share of mapped wall more than 10 cm from a
    /// true one is 6.3 % at two metres, **2.2 % at three**, 3.9 % at four;
    /// doubled walls 0.6 / 0.3 / 0.8 %; coverage of the true wall surfaces
    /// 44 / 48 / 47 %. Three beats two on six of the eight recordings and
    /// four beats three on none of the aggregates — noise does start to
    /// cost past three, just not before it.
    ///
    /// The two-metre limit is why the far half of an open room never
    /// reached the map: the sensor sees four metres and the mapper was
    /// keeping the nearest two. In the flat with a wide bay the difference
    /// is the whole finding — 14.4 % of its walls misplaced at two metres,
    /// 1.2 % at three, with coverage going from 48 % to 62 %.
    ///
    /// **A caveat that belongs with the number.** This is the simulated
    /// sensor's noise: 3 mm plus 20 mm at four metres, growing linearly.
    /// A real VL53L8 in daylight is a poorer instrument than that, and the
    /// original two metres may well be right for it. Before this becomes a
    /// proposal to anyone, it needs the real sensor's noise at three
    /// metres, which is hardware's answer and not the twin's.
    pub max_range_m: f32,
