...
pub struct RelocalizeConfig {
    ...
    /// Uniqueness: the winner is accepted only when its residual is at
    /// most this fraction of the runner-up basin's (a distinct seed,
    /// ≥ 0.4 m / 35° away). Measured on the twin: in a symmetric flat two
    /// basins scored 0.013 and 0.020 and the wrong one won; a pose that
    /// barely beats another is ambiguity, not a pose.
...
pub struct RelocalizeResult {
    ...
    /// The best refined pose in another basin, and its residual — what
    /// the uniqueness test compares against. `None` with one seed only.
    pub runner_up: Option<((f32, f32, f32), f32)>,
    /// Every basin the search thinks plausible, best first, the winner
    /// included: one still window of an 8×8 ToF in a flat of repeated
    /// rectangles cannot tell them apart, so the caller may keep them all
    /// and let a second viewpoint decide (2026-09-09).
    pub basins: Vec<((f32, f32, f32), f32)>,
...
pub struct PoseAgreement {
    ...
    pub mean_residual_m: f32,
    /// The same over the LONG beams alone (endpoint beyond
    /// [`LONG_BEAM_M`] from the body), and how many there were. A
    /// kidnap contradicts long and short beams alike; a low thing seen
    /// close — a bed 20 cm high, a metre off — contradicts the short
    /// ones only, and is not a lost pose (quacksat's explmap4,
    /// 2026-09-19: lost for good in a bedroom with the pose right).
    pub long_residual_m: f32,
    pub n_long: u32,
...
/// A beam is long past this endpoint range: what a low thing close to
/// the body cannot reach.
pub const LONG_BEAM_M: f32 = 0.7;
...
pub fn score_pose(
    ...
    score_pose_with(grid, scan, pose, clamp_m, wall_threshold_fp, observed_fp, None)
}

/// [`score_pose`] for a pose that is NOT trusted — a relocalize
/// candidate, a boot hypothesis, an alias under test — with the term the
/// trusted-pose judge deliberately lacks: **a beam may not pass through a
/// confident wall on its way to its endpoint.** The endpoint distance
/// alone is blind along the ray: a pose in the next room whose every
/// beam crosses the shared wall to land on the far wall of the room the
/// robot is actually in scores exactly like the truth — which is the
/// mirror image, the identical neighbour, the corridor's 180° twin, every
/// alias that got confirmed on the twin (2026-09-14/15). The search has a
/// one-sample midpoint test; a candidate's confirmation had none, so it
/// was nominated by a judge with one eye and confirmed by one with none.
///
/// Here the ray is walked cell by cell (Bresenham, the same walk that
/// inked it), and a cell above `see_through_fp` strictly before the last
/// two cells of the ray counts the beam as a contradiction at the clamp
/// and as observed. The two-cell grace is the search's graze exemption:
/// a beam that ends on a wall at close range or grazing incidence passes
/// through that wall's own cells first, and must not be punished for
/// hitting the very wall it measured (the false-LOST of field test four).
/// Cost: one lookup per ray cell — fine for a few judgements per window,
/// which is what this is for; the search keeps its midpoint sample.
#[allow(clippy::too_many_arguments)]
pub fn score_pose_rays(
    grid: &mut OccupancyGrid,
    scan: &Scan,
    pose: (f32, f32, f32),
    clamp_m: f32,
...
fn score_pose_with(
    ...
    let (mut long_sum, mut n_long) = (0.0_f32, 0u32);
    for &((obx, oby), (bx, by)) in &scan.beams {
        n_beams += 1;
        let is_long = (bx - obx).hypot(by - oby) >= LONG_BEAM_M;
        ...
        if let Some(fp) = see_through_fp {
            let ox = pose.0 + cy * obx - sy * oby;
            let oy = pose.1 + sy * obx + cy * oby;
            let oj = ((ox - x_min) / cell).floor() as i32;
            let oi = ((oy - y_min) / cell).floor() as i32;
            let mut crossed = false;
            crate::grid::bresenham(oi, oj, i, j, |ci, cj, _at_end| {
                if crossed || ci < 0 || cj < 0 || (ci as usize) >= h || (cj as usize) >= w {
                    return;
                }
                // The last two cells before the endpoint are the
                // endpoint's own wall: exempt.
                if (ci - i).abs() <= 2 && (cj - j).abs() <= 2 {
                    return;
                }
                if log[(ci as usize) * w + (cj as usize)] > fp {
                    crossed = true;
                }
            });
            if crossed {
        ...
        let r = field[(i as usize) * w + (j as usize)].min(clamp_m);
        sum += r;
        n_observed += 1;
        if is_long {
            long_sum += r;
    ...
    PoseAgreement {
        ...
        long_residual_m: if n_long > 0 { long_sum / n_long as f32 } else { 0.0 },
        n_long,
        n_observed,
        n_beams,
    }
...
pub fn relocalize_against_grid(
    ...
    // Phase 2 — local refinement around every seed; global best wins, and
    // the best of the OTHER seeds is kept as the runner-up.
    let mut best: Option<RelocalizeResult> = None;
    let mut per_seed: Vec<RelocalizeResult> = Vec::with_capacity(seeds.len());
    ...
    for seed in &seeds {
        let mut seed_best: Option<RelocalizeResult> = None;
        ...
        for yi in 0..yaw_bins {
            ...
            for ddi in -r_xy..=r_xy {
                for ddj in -r_xy..=r_xy {
                    ...
                    if seed_best.as_ref().is_none_or(|b| mean < b.mean_residual_m) {
                        seed_best = Some(cand.clone());
                    }
    ...
    // The runner-up: the best refined pose of a seed in another basin.
    best.runner_up = per_seed
        .iter()
        .filter(|r| {
            let dx = r.pose.0 - best.pose.0;
            let dy = r.pose.1 - best.pose.1;
            let dyw = wrap_pi(r.pose.2 - best.pose.2).abs();
            dx * dx + dy * dy >= MIN_SEP_M * MIN_SEP_M || dyw >= MIN_SEP_YAW
        })
        .min_by(|a, b| a.mean_residual_m.total_cmp(&b.mean_residual_m))
        .map(|r| (r.pose, r.mean_residual_m));
    // Every distinct basin, best first — the winner and its rivals.
    {
        let mut basins: Vec<((f32, f32, f32), f32)> = Vec::new();
        for r in std::iter::once(&best).chain(per_seed.iter()) {
            let far = basins.iter().all(|(p, _)| {
                let (dx, dy) = (p.0 - r.pose.0, p.1 - r.pose.1);
                dx * dx + dy * dy >= MIN_SEP_M * MIN_SEP_M
                    || wrap_pi(p.2 - r.pose.2).abs() >= MIN_SEP_YAW
            });
            if far {
                basins.push((r.pose, r.mean_residual_m));
            }
        }
        basins.sort_by(|a, b| a.1.total_cmp(&b.1));
        basins.truncate(MAX_BASINS);
        best.basins = basins;
    }
    let unique = best
        .runner_up
        .is_none_or(|(_, r)| best.mean_residual_m <= cfg.uniqueness_ratio * r);
    // What the search actually had to work with, for the debug line below.
    let probe_beams = scan.endpoints_body().count().max(1) as f32;
    if std::env::var_os("RELOC_DEBUG").is_some() {
        eprintln!(
            "    reloc: best ({:.2}, {:.2}, {:.0}°) residual {:.4} beams {}/{} share {:.2} runner-up {:?}",
            best.pose.0, best.pose.1, best.pose.2.to_degrees(), best.mean_residual_m,
            best.n_beams_used, probe_beams as u32, best.n_beams_used as f32 / probe_beams,
            best.runner_up.map(|(p, r)| (format!("({:.2},{:.2})", p.0, p.1), r)),
        );
    }
...
mod tests {
    ...
    /// Two rooms side by side, the right one a metre narrower, a 360° scan
    /// taken in the left one. Put the scan in the right room at the same
    /// distance from the shared wall: the beams toward the shared wall
    /// agree, and the beams the other way run THROUGH the right room's far
    /// wall to end in territory the map never saw. The endpoint judge
    /// skips those as unjudgeable and is left with the agreeing half — the
    /// office-fits-the-bedroom alias exactly. The ray judge sees the wall
    /// they crossed.
    #[test]
    fn the_ray_judge_sees_through_the_far_wall() {
        let mut g = OccupancyGrid::new(GridConfig {
            x_range: (-4.5, 4.5),
            y_range: (-2.5, 2.5),
            cell: 0.05,
        });
        // Left room x ∈ [-4, 0], right room x ∈ [0, 3], y ∈ [-2, 2]; each
        // inked from its own centre so no ray crosses the shared wall
        // while drawing it.
        let n = 400;
        for _ in 0..4 {
            for k in 0..n {
                ...
                let t = -4.0 + 4.0 * f;
                g.integrate_ray(-2.0, 0.0, t, 2.0, true);
                g.integrate_ray(-2.0, 0.0, t, -2.0, true);
                let tr = 3.0 * f;
                g.integrate_ray(1.5, 0.0, tr, 2.0, true);
                g.integrate_ray(1.5, 0.0, tr, -2.0, true);
                let u = -2.0 + 4.0 * f;
                g.integrate_ray(-2.0, 0.0, -4.0, u, true);
                g.integrate_ray(-2.0, 0.0, 0.0, u, true);
                g.integrate_ray(1.5, 0.0, 3.0, u, true);
                g.integrate_ray(1.5, 0.0, 0.0, u, true);
            }
        }
        // A 360° scan from (-2, 0): ranges to the left room's walls.
        let (mut angles, mut ranges) = (Vec::new(), Vec::new());
        for k in 0..180 {
            let a = -std::f32::consts::PI + k as f32 * (2.0 * std::f32::consts::PI / 180.0);
            let (dx, dy) = (a.cos(), a.sin());
            let tx = if dx > 1e-6 { 2.0 / dx } else if dx < -1e-6 { -2.0 / dx } else { f32::INFINITY };
            let ty = if dy > 1e-6 { 2.0 / dy } else if dy < -1e-6 { -2.0 / dy } else { f32::INFINITY };
            angles.push(a);
            ranges.push(tx.min(ty));
        }
        let scan = Scan::from_polar(&angles, &ranges, (0.0, 0.0), 0.05);
        let truth = (-2.0, 0.0, 0.0);
        // The alias: 2 m from the shared wall inside the right room, whose
        // far wall is only 1 m beyond it.
        let alias = (2.0, 0.0, 0.0);
        let eyes_shut_truth = score_pose(&mut g, &scan, truth, 0.5, 200, 50);
        let eyes_shut_alias = score_pose(&mut g, &scan, alias, 0.5, 200, 50);
        assert!(eyes_shut_truth.mean_residual_m < 0.05, "{eyes_shut_truth:?}");
        assert!(
            eyes_shut_alias.mean_residual_m < 0.05,
            "the endpoint judge is expected to be fooled: {eyes_shut_alias:?}"
        );
        let rays_truth = score_pose_rays(&mut g, &scan, truth, 0.5, 200, 50, 300);
        let rays_alias = score_pose_rays(&mut g, &scan, alias, 0.5, 200, 50, 300);
        assert!(rays_truth.mean_residual_m < 0.05, "the truth crosses nothing: {rays_truth:?}");
        assert!(
