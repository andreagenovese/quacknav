...
pub struct MclConfig {
    ...
    /// What a beam whose endpoint lands on a cell the map has no opinion
    /// about (|log| below `observed_fp`) contributes: `None` scores it by
    /// the distance to the nearest known wall, as if the map were complete
    /// — which, on a map built by a duck through a 45° wedge, punishes
    /// every beam into the unexplored half of a room and drags the cloud
    /// toward whatever is mapped. `Some(r)` scores it a flat `r` (the beam
    /// sigma is the honest choice: "no evidence"), and such beams are left
    /// out of the lock residual, as `relocalize::score_pose` leaves them
    /// out of its agreement.
    pub unknown_residual_m: Option<f32>,
    pub observed_fp: i16,
...
impl Localizer {
    ...
    pub fn update(&mut self, grid: &mut OccupancyGrid, scan: &Scan) {
        ...
        let unknown = self.cfg.unknown_residual_m;
        let observed_fp = self.cfg.observed_fp;
        // The distance, and whether the map could judge this beam at all.
        let endpoint_j = |px: f32, py: f32, cy: f32, sy: f32, bx: f32, by: f32| -> (f32, bool) {
            let ex = cy * bx - sy * by;
            let ey = sy * bx + cy * by;
            let hx = px + ex;
            let hy = py + ey;
            ...
            if let Some(r) = unknown
                && log[(i as usize) * w + (j as usize)].abs() < observed_fp
            {
                return (r, false);
            }
            (field[(i as usize) * w + (j as usize)].min(clamp), true)
        };
        let endpoint_d = |px: f32, py: f32, cy: f32, sy: f32, bx: f32, by: f32| -> f32 {
            endpoint_j(px, py, cy, sy, bx, by).0
        };
            ...
            .max_by(|a, b| a.1.partial_cmp(b.1).unwrap_or(std::cmp::Ordering::Equal))
        {
            ...
            let (sy, cy) = p.2.sin_cos();
            // Over the beams the map could judge: an unmapped half-room is
            // no evidence for or against, and must not hold the lock open
            // or shut.
            let (mut sum, mut n) = (0.0_f32, 0usize);
            for &(bx, by) in &beams {
                let (d, judged) = endpoint_j(p.0, p.1, cy, sy, bx, by);
                if judged {
                    sum += d;
                    n += 1;
                }
            }
            self.last_residual_m = if n > 0 { sum / n as f32 } else { clamp };
        }
