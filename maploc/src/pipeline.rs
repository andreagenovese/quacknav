    ...
    fn default() -> Self {
        Self {
            ...
            loops: LoopCloserConfig {
                ...
                // Odometry drifts a few percent of travel, and a submap is
                // 0.8 m: the stock 0.08 m per submap admitted corrections
                // ten times that, and on recorded sessions those were the
                // aliased ones (maploc-audit, 2026-09-06).
                max_correction_per_submap_m: 0.03,
                max_correction_cap_m: 0.30,
                // A closure knows less than it claims. The stock 5 cm / 3°
                // tells the optimizer that two submaps' relative pose is
                // known to a cell and a half, so the graph bends to satisfy
                // every closure — including the ones a repeated rectangle
                // produced — and the map ends up smeared: a fifth of its
                // walls more than 10 cm from the true one, a twentieth
                // drawn twice. Widened to 40 cm / 14°, closures still pull
                // drift out (with none at all the map is worse) but no
                // longer outvote odometry and the still windows.
                //
                // Measured by replaying six recorded sessions — two
                // simulated flats, half an hour each — and scoring every
                // resulting map against the house's own walls: better on
                // all six, on both counts, walls beyond 10 cm falling from
...
impl Slam {
    ...
    /// Cut the current submap at the next tick — see
    /// [`crate::submap_manager::SubmapManager::request_switch`].
    pub fn request_submap_switch(&mut self) {
        self.mgr.request_switch();
    }
