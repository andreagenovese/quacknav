...
fn main() {
    ...
    let mut tally: std::collections::BTreeMap<&'static str, u32> = std::collections::BTreeMap::new();
    let mut correction_m = 0.0f64;
    let (mut agree_sum, mut agree_n, mut agree_bad) = (0.0f64, 0u32, 0u32);
    let mut loop_shift_m = 0.0f64;
    let rp = Reprojector::alpha();
    // Bench knobs (env), for hypothesis testing on recorded sessions.
    let envf = |k: &str, d: f32| std::env::var(k).ok().and_then(|v| v.parse().ok()).unwrap_or(d);
    let mut slam_cfg = SlamConfig::default();
    slam_cfg.loops.radius_m = envf("LOOP_RADIUS", slam_cfg.loops.radius_m);
    ...
    slam_cfg.loops.min_index_gap = envf("LOOP_GAP", slam_cfg.loops.min_index_gap as f32) as usize;
    slam_cfg.loops.verbose = std::env::var_os("LOOP_VERBOSE").is_some();
    slam_cfg.optimizer.huber_delta = envf("HUBER", slam_cfg.optimizer.huber_delta);
    slam_cfg.optimizer.reject_sigmas = envf("REJECT", slam_cfg.optimizer.reject_sigmas);
    slam_cfg.optimizer.reject_max = envf("REJECT_MAX", slam_cfg.optimizer.reject_max as f32) as u32;

    slam_cfg.odom_sigma_xy = envf("ODOM_SIGMA_XY", slam_cfg.odom_sigma_xy);
    slam_cfg.odom_sigma_yaw = envf("ODOM_SIGMA_YAW", slam_cfg.odom_sigma_yaw);
    let mut mapper_cfg = MapperConfig::default();
    // `ACC_RANGE`: how far a beam may be and still reach the map. The
    // accumulator drops longer ones outright, which is why the far half of
    // an open room never gets inked.
    mapper_cfg.accumulator.max_range_m = envf("ACC_RANGE", mapper_cfg.accumulator.max_range_m);
    mapper_cfg.tracking.enabled = std::env::var("TRACK").map(|v| v == "1").unwrap_or(mapper_cfg.tracking.enabled);
    mapper_cfg.tracking.max_correction_m = envf("TRACK_MAX", mapper_cfg.tracking.max_correction_m);
    mapper_cfg.tracking.prior_sigma_xy = envf("TRACK_PRIOR_XY", mapper_cfg.tracking.prior_sigma_xy);
    mapper_cfg.tracking.min_conditioning = envf("TRACK_COND", mapper_cfg.tracking.min_conditioning);
    mapper_cfg.tracking.min_improvement = envf("TRACK_IMPROVE", mapper_cfg.tracking.min_improvement);
    mapper_cfg.tracking.cut_on_correction_m = envf("CUT_ON_CORR", mapper_cfg.tracking.cut_on_correction_m);
    mapper_cfg.tracking.max_residual_after_m = envf("TRACK_MAX_AFTER", mapper_cfg.tracking.max_residual_after_m);
    mapper_cfg.tracking.min_residual_before_m = envf("TRACK_MIN_BEFORE", mapper_cfg.tracking.min_residual_before_m);
    mapper_cfg.relocalize_agree_windows = envf("RELOC_AGREE", mapper_cfg.relocalize_agree_windows as f32) as u32;
    let max_t = envf("MAX_T", f32::INFINITY);
    // `MAP_SESSION=<file>` replays into a map saved by an earlier run,
    // starting lost: the boot-relocalization question, on the bench.
    let mut mapper = match std::env::var_os("MAP_SESSION") {
        Some(path) => {
            let session = maploc::session::SessionState::load(std::path::Path::new(&path))
                .expect("read the saved session")
                .expect("the saved session is empty");
            eprintln!(
                "loaded {} — {} frozen submaps, saved pose ({:.2}, {:.2}, {:.1}°); starting LOST",
                std::path::Path::new(&path).display(),
                session.frozen.len(),
                session.tracked.0,
                session.tracked.1,
                session.tracked.2.to_degrees(),
            );
            Mapper::resumed_lost(mapper_cfg, Slam::from_session(slam_cfg, session))
        }
        None => Mapper::new(mapper_cfg, Slam::new(slam_cfg)),
    };
    // Tracked-vs-raw-odometry drift: on the MuJoCo twin raw odometry is within
    // ~0.1 m / 2 deg of truth over 40 m, so this is the correction maploc added.
    let (mut drift_sum, mut drift_n, mut drift_max) = (0.0f32, 0u32, 0.0f32);
    ...
    for record in SessionReplayer::open(&session).expect("open session") {
        ...
        match record {
            ...
            Record::Tof(frame) => {
                ...
                let flat = maploc::flat::flatten(&rp, &ranges, head, &posture);
        ...
        for note in notes.drain(..) {
            let key = match &note {
                ...
                Note::RelocalizeCandidate { .. } => "reloc_candidate",
            ...
            *tally.entry(key).or_insert(0u32) += 1;
            if let Note::WindowIntegrated { mean_residual_m, n_observed, .. } = &note
                && *n_observed > 0
            {
                agree_sum += *mean_residual_m as f64;
                agree_n += 1;
                if *mean_residual_m > 0.08 {
                    agree_bad += 1;
                }
            }
            if let Note::TrackingCorrected { dx, dy, .. } = &note {
                correction_m += (dx * dx + dy * dy).sqrt() as f64;
            }
            if let Note::LoopClosed { dx, dy, .. } = &note {
                loop_shift_m += (dx * dx + dy * dy).sqrt() as f64;
            }
            ...
            match note {
                Note::WindowIntegrated {
                    ...
                    let vs_truth = mapper
                        ...
                        .unwrap_or(f32::NAN);
                    // The same composite scored at the raw-odometry pose (map
                    // frame = odometry frame until a closure moves it).
                    let od = match (first_odom, latest_odom.as_ref()) {
                        (Some(f), Some(o)) => between(f, (o.odom_x, o.odom_y, o.odom_yaw)),
                        _ => (f32::NAN, f32::NAN, f32::NAN),
                    };
                    let vs_truth_odom = mapper
                        .last_window()
                        .map(|(p, s)| {
                            // Move the composite from the tracked pose to the odom pose.
                            let _ = p;
                            truth.scan_agreement(od, s)
                        })
                        .unwrap_or(f32::NAN);
                    let drift = (tr.0 - od.0).hypot(tr.1 - od.1);
                    drift_sum += drift;
                    drift_n += 1;
                    drift_max = drift_max.max(drift);
                    println!(
                        "[{t:7.1}s] window {beams:5} beams  vs-map {mean_residual_m:.3} ({n_observed:5}/{n_beams:5})  vs-TRUTH {vs_truth:.3} @odom {vs_truth_odom:.3}  tracked ({:6.2}, {:6.2}, {:6.1}°)  odom ({:6.2}, {:6.2}, {:6.1}°)  drift {drift:.2}",
                        tr.0,
                        tr.1,
                        tr.2.to_degrees(),
                        od.0,
                ...
                Note::TrackingCorrected {
                    ...
                    println!(
                        "[{t:7.1}s] tracking corrected ({dx:.3}, {dy:.3}, {:.1}°) residual {residual_before_m:.3} -> {residual_after_m:.3} over {n_beams_used} beams",
                        dyaw.to_degrees()
                    );
                }
    ...
    if drift_n > 0 {
        println!(
            "tracked vs raw odometry: mean {:.3} m, max {:.3} m over {} windows",
            drift_sum / drift_n as f32,
            drift_max,
            drift_n
        );
    }
    ...
    if let Some(grid) = mapper.slam().render() {
        ...
        // What the mapper did over the run, on one line: a bad map and a
        // good one from the same house differ somewhere in here.
        println!(
            "AGREE mean={:.4} bad_share={:.3} n={agree_n}",
            if agree_n > 0 { agree_sum / agree_n as f64 } else { 0.0 },
            if agree_n > 0 { agree_bad as f64 / agree_n as f64 } else { 0.0 },
        );
        println!(
            "TALLY {} corrected_m={correction_m:.2} loop_shift_m={loop_shift_m:.2}",
            tally
                .iter()
                .map(|(k, v)| format!("{k}={v}"))
                .collect::<Vec<_>>()
        ...
        // `OUT_SESSION=<file>`: the map this replay built, saved the way
        // the robot saves one — so a bench run and a live run are scored
        // by the same tools (quacksat's mapquality.py, dump_frame).
        if let Some(path) = std::env::var_os("OUT_SESSION") {
            match mapper.slam().save(std::path::Path::new(&path)) {
                Ok(()) => println!("session saved to {}", std::path::Path::new(&path).display()),
                Err(e) => println!("could not save the session: {e}"),
            }
        }
