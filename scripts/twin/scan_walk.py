"""Stop-and-scan: short walking bursts separated by stands.
    ...
    scan_walk.py <secs> <robotd.sock> <sim_port>

From Peter Schade's sim-maploc/scan_walk.py (Pollen Robotics,
pollen-robotics/microduck PR 202, Apache-2.0), with the sockets and the
simulator's port as arguments: the same legs on two twins side by side is
the A/B of ab_round.sh.
...
b = socket.socket(); b.connect(("127.0.0.1", int(sys.argv[3])))
bf = b.makefile("rw")
